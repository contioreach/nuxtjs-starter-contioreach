/* Article body preparation, shared by the renderer and the table of contents
   so the two always agree on heading ids. This runs once on the server (see
   server/api/posts/[slug].get.js) and both the HTML and the TOC are handed to
   the page, so the browser never re-parses the article. */
